...
NO* percorrer(NO* p){
	...
	while ( pp->dir != NULL &&  pp->esq->chave < pp->dir->chave ){
		printf("%d  ", pp->esq->chave);
		pp = pp->dir;
	}

	printf("chave errado : %d\n", pp->esq->chave);
	return pp->esq;
...
NO* ehValido(NO* *raiz, NO* *queueue){
	...
	NO* numero = validaaa(*raiz, queueue);
	if (numero == NULL) return NULL;
	return numero;
	
	//NO* p = acharNo(*raiz, numero);
	//montarQueue(queueue, numero);
...
void organizar(NO* *raiz){
	...
	if( (*raiz) != NULL ){
		...
		//imprimirArvore(queueue, 0);

		NO* errado = percorrer(queueue);
		printf("ERRO RETORNADO : %d\n\n", errado->chave);
		if (errado != NULL) exlcuirNo(raiz, errado);
...
// codigo copiado
NO* ehValido2(NO* *raiz, NO* *queueue){
...
/*
	NO* yi = inserirNo(&arv, 1);
	NO* si = inserirDadoPai(&arv, yi, -8, 2);
	NO* san = inserirDadoPai(&arv, si, 3, 1);
	NO* liu = inserirDadoPai(&arv, si, 6, 2);
*/
...
// nao eh caso valido
	...
	NO* ocho = inserirDadoPai(&arv, jiushi, 10, 2);
*/
