...
NO* percorrer(NO* p, NO* raiz){
    ...
    if (outraFila != NULL){
        ...
        NO* menorAlt = NULL;
	    while( pOutro->dir ){
           if ( altura(pOutro->esq) > altura(pOutro->dir->esq)) menorAlt = pOutro->esq;
            else menorAlt = pOutro->dir->esq;
            pOutro = pOutro->dir;
        }
/*
        // if leaf, remove leaf
        NO* ppOutro = outraFila;
        while(ppOutro){
            if (ppOutro->esq->esq == NULL && ppOutro->esq->dir == NULL) menorAlt = ppOutro->esq;
            ppOutro = ppOutro->dir;
        }

        if( menorAlt->esq == NULL && menorAlt->dir == NULL ){
        NO* pppOutro = outraFila;
        while(pppOutro){
            if (pppOutro->esq->chave == raiz->chave) menorAlt = pppOutro->esq;
            pppOutro = pppOutro->dir;
        }     
        }  
*/
        if (menorAlt == NULL) return pOutro->esq;
        return menorAlt;
    }
